...
//					g.drawInteractive(gr);
		...
		public void collectOutcomeOfExperiments(processSubExperimentResult<RESULT> handlerForExperimentResults)
		{
			...
			if (nameToGraph.size() != handlerForExperimentResults.getGraphs().length)
				throw new IllegalArgumentException("duplicate file names in some graphs"+nameToGraph);
					...
					for(int rCounter=taskCounterFromPreviousSubExperiment;rCounter < taskCounter;++rCounter)
					{
						...
						try
						{
							...
							while(line != null)
							{
								...
								System.out.println(nameToGraph);
...
//									thisPlot.addPairValues((Comparable)argValue, yValue, color, label);
							    	thisPlot.addExcel((Comparable)argValue, yValue, color, label);

								
								else
									thisPlot.add((Comparable)argValue, yValue, color, label);*/

//								else if(!thisPlot.getFileName().startsWith("RBoxPlotCvs") && thisPlot.getFileName().endsWith(".csv"))
//								{
//									thisPlot.addPairValues((Comparable)argValue, yValue, color, label);	
//								}
//								
//								else 
								...
								if(!thisPlot.getFileName().startsWith("RBoxPlotCvs") && thisPlot.getFileName().endsWith(".csv"))
								{
									thisPlot.addPairValues((Comparable)argValue, yValue, color, label);	
		...
		public void Record(RGraph graph, Comparable x, Double y, String colour, String label) throws IOException
		{
			...
			case RUN_STANDALONE:	
				if(!graph.getFileName().startsWith("RBoxPlotCvs") && graph.getFileName().endsWith(".csv"))
				{
				...
				if(graph.getFileName().startsWith("RBoxPlotCvs") && graph.getFileName().endsWith(".csv"))
				{
					graph.addExcel(x,y,colour,label);			
				}
