...
//			LearnerEvaluationConfiguration learnerEval = new LearnerEvaluationConfiguration(config);learnerEval.setLabelConverter(converter);
			...
			{// try learning the same machine a few times
				...
				learnerList.add(new ReferenceLearnerUsingSiccoScoring(referenceLearnerEval,ptaCopy,true));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new ReferenceLearnerUsingSiccoScoring(referenceLearnerEval,ptaCopy,false));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new KTailsReferenceLearner(referenceLearnerEval,ptaCopy,true,1));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new KTailsReferenceLearner(referenceLearnerEval,ptaCopy,true,2));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new KTailsReferenceLearner(referenceLearnerEval,ptaCopy,false,1));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new KTailsReferenceLearner(referenceLearnerEval,ptaCopy,false,2));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new EDSMReferenceLearner(referenceLearnerEval,ptaCopy,1));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new EDSMReferenceLearner(referenceLearnerEval,ptaCopy,2));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new EDSMReferenceLearner(referenceLearnerEval,ptaCopy,3));
				ptaCopy = new LearnerGraph(deepCopy);LearnerGraph.copyGraphs(pta, ptaCopy);
				learnerList.add(new EDSMReferenceLearner(referenceLearnerEval,ptaCopy,4));
	...
	public static class ReferenceLearnerUsingSiccoScoring extends LearnerThatCanClassifyPairs
	{
		...
		public void setquestioPTA(LearnerGraph ptaCopy1) {
			questioncoregraph=ptaCopy1;			
		}
		
		public LearnerGraph getQuestionPTA() {
			return this.questioncoregraph;		
		}
		protected final boolean scoringSiccoRecursive;
		List<List<Label>> positive=new ArrayList<List<Label>>();
		List<List<Label>> negative=new ArrayList<List<Label>>();
		...
		public void setReference(LearnerGraph ref) {
			referenceGraph=ref;			
		}
		public void setPositivesNegatives(List<List<Label>> sPlus, List<List<Label>> sMinus) {
			positive=sPlus;
			negative=sMinus;
...
//			return "EDSM,>="+config.getRejectPositivePairsWithScoresLessThan();
	...
	public static void runExperiment() throws Exception
	{
		...
		*/
		final int traceQuantity=10;// half of those generated will be negative that will be thrown away in most experiments.
