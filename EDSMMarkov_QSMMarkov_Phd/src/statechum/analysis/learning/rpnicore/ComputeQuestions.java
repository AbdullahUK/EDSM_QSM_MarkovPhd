...
//		public SequenceSet getPathsToRedMergednode();
			...
			final LearnerGraph original, final LearnerGraph learnt,final QuestionConstructor qConstructor)
	{
		...
		for(AMEquivalenceClass<CmpVertex,LearnerGraphCachedData> eq:learnt.learnerCache.getMergedStates())
			...
			/*	@Override 
				public SequenceSet getPathsToRedMergednode() 
				{
					SequenceSet toMergenode = engine.new SequenceSet();
					original.pathroutines.computePathsSBetween(original.getInit(), 
							//original.findVertex(learnt.learnerCache.stateLearnt.getStringId()), 
							pairToMerge.getR(),
							identity, toMergenode);
					return toMergenode;
				}*/
	...
	public static PTASequenceEngine computeQS_QSM(final StatePair pairToMerge, 
			final LearnerGraph original, final LearnerGraph learnt,final QuestionConstructor qConstructor)
	{
		final PTASequenceEngine engine = qConstructor.constructEngine(original, learnt);
		...
		for(AMEquivalenceClass<CmpVertex,LearnerGraphCachedData> eq:learnt.learnerCache.getMergedStates())
			qConstructor.addQuestionsForState(eq, original, learnt, pairToMerge, 
				...
				public SequenceSet getPathsToLearnt() 
				{
					...
					/*System.out.println("reds= "+pairToMerge.getR()+" "+toMergenode.getDebugData());
					Visualiser.updateFrame(original, null);*/
					/*if(toMergenode.getSize() > 1)
					{
					...
					/*if(!learnt.learnerCache.stateLearnt.getStringId().equals(pairToMerge.getR().getStringId()))
					{
						
						System.out.println("pair= "+pairToMerge);
						System.out.println("red= "+pairToMerge.getR());
						System.out.println("learntstate= "+learnt.learnerCache.stateLearnt);
						System.out.println("mergedvertices= "+learnt.learnerCache.getMergedStates());
						...
						MergeStates.mergeAndDeterminize_general(original, pairToMerge);
												
						Visualiser.updateFrame(original, learnt);
					}*/
					return toMergenode;
	...
	/**  QSM question generator of Abdullah . */
	static public class ABDULLAHQuestionGenerator implements QuestionConstructor
	{
		private PTASequenceEngine engine = null;
		private Map<CmpVertex,PTASequenceEngine.SequenceSet> fanout = null;
		private Map<CmpVertex,PTASequenceEngine.SequenceSet> bluefanout = null;
...
//				System.out.println("state= "+state);
				SequenceSet pathsToMergedRed = data.getPathsToLearnt();
				SequenceSet pathsToBlue = data.getPathsToBlue();
...
//				pathsToMergedRed.unite(pathsToMergedRed.crossWithSet(inputsToMultWith));// the resulting path does a "transition cover" on all transitions leaving the red state.
				...
				pathsToMergedRed.limitTo(original.config.getQuestionPathUnionLimit());
				fanout = learnt.pathroutines.computePathsSBetween_All(stateLearnt, engine, pathsToMergedRed);
				bluefanout= learnt.pathroutines.computePathsSBetween_All(stateLearnt, engine, pathsToBlue);
			}
			...
			if (pathsToCurrentState != null)
			{
				...
				// if a path from the merged red state to the current one can be found, update the set of questions. 
//				pathsToCurrentState.crossWithMap(learnt.transitionMatrix.get(state.getMergedVertex()));
				assert stateLearnt.getColour()!=JUConstants.BLUE;
				pathsToCurrentState.crossWithMap(learnt.transitionMatrix.get(state.getMergedVertex()));

//				System.out.println(pathsToCurrentState.getDebugData());
				// Note that we do not care what the result of crossWithSet is - for those states which 
				// do not exist in the underlying graph, reject vertices will be added by the engine and
				// hence will be returned when we do a .getData() on the engine.
			}
			
			SequenceSet pathsToblue = bluefanout.get(state.getMergedVertex());
...
//				System.out.println(pathsToCurrentState.getDebugData());
	...
	/**  QSM question generator of DUPONT's paper. */
	static public class DUPONTQuestionGenerator implements QuestionConstructor
	{
		private PTASequenceEngine engine = null;
		private Map<CmpVertex,PTASequenceEngine.SequenceSet> fanout = null;
...
//				pathsToMergedRed.unite(pathsToMergedRed.crossWithSet(inputsToMultWith));// the resulting path does a "transition cover" on all transitions leaving the red state.
				...
				// not be returned.
				pathsToMergedRed.limitTo(original.config.getQuestionPathUnionLimit());
//				System.out.println(pathsToMergedRed.getDebugData());
				fanout = learnt.pathroutines.computePathsSBetween_All(stateLearnt, engine, pathsToMergedRed);
...
//				System.out.println("aftercross"+pathsToCurrentState.getDebugData());
				// Note that we do not care what the result of crossWithSet is - for those states which 
				// do not exist in the underlying graph, reject vertices will be added by the engine and
				// hence will be returned when we do a .getData() on the engine.
			}
...
//				SequenceSet pathsToMergedRed = data.getPathsToLearnt();
//				
//				// this to get paths to merged red in the original graph
////				SequenceSet pathsToMergedRed = data.getPathsToRedMergednode();
//				
//				// Now we limit the number of elements in pathsToMerged to the value specified in the configuration.
//				// This will not affect the underlying graph, but it does not really matter since all
//				// elements in that graph are accept-states by construction of pathsToMergedRed and hence
...
// in the original one, but will not be found because they are not the shortest ones. 
	...
	public static PTASequenceEngine getQuestionPta(final StatePair pair, LearnerGraph original, LearnerGraph merged, LearnerGraph [] properties)
	{
		...
		switch(original.config.getQuestionGenerator())
		{
			...
			case DUPONT:qConstructor=new DUPONTQuestionGenerator();break;
			case ABDULLAH:qConstructor=new ABDULLAHQuestionGenerator();break;
		...
		if (engine == null)
		{
			if(original.config.getQuestionGenerator()==QuestionGeneratorKind.DUPONT)
			engine = computeQS_QSM(pair, original, merged, qConstructor);
			else
